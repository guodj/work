...
for v in range(4, 8):
    nn += np.array(data[v][iLon0, sLat:eLat, sAlt:eAlt])

rho = np.array(data[3][iLon0, sLat:eLat, sAlt + 1:eAlt - 1])
temp = np.array(data[15][iLon0, sLat:eLat, sAlt:eAlt])
gp = np.array([])
gp.resize(nLats, nAlts - 2)
d2db = np.array([])
d2db.resize(nLats, nAlts - 2)

for ih in range(0, nAlts - 2):
    gp[:, ih] = (get_p(nn[:, ih + 2], temp[:, ih + 2]) - get_p(nn[:, ih], temp[:, ih])) / (dh[ih] * 1000)
    d2db[:, ih] = -gp[:, ih] / rho[:, ih] - get_g(hs[ih] * 1000)

norm = cm.colors.Normalize(vmax=0.45, vmin=-0.45)
...
for file in filelist:
    ...
    rho = np.array(data[3][iLon, sLat:eLat, sAlt + 1:eAlt - 1])
    temp = np.array(data[15][iLon, sLat:eLat, sAlt:eAlt])
    gp = np.array([])
    gp.resize(nLats, nAlts - 2)
    d2d = np.array([])
    d2d.resize(nLats, nAlts - 2)

    for ih in range(0, nAlts-2):
        gp[:, ih] = (get_p(nn[:, ih + 2], temp[:, ih + 2]) - get_p(nn[:, ih], temp[:, ih])) / (dh[ih] * 1000)
        d2d[:, ih] = -gp[:, ih] / rho[:, ih] - get_g(hs[ih] * 1000)
