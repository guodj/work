...
Var = header["vars"][args["var"]].decode().replace(" ","")
...
#plt.figure()
#
#maxi  = np.max(AllData)*1.05
#mini  = np.min(AllData)*0.95
#dr = (maxi-mini)/31
#
#levels = np.arange(mini, maxi, dr)
#norm = cm.colors.Normalize(vmax=np.max(AllData), vmin=np.min(AllData))
#cmap = cm.rainbow
#
#CS = plt.contourf(AllTimes, AllAlts, np.transpose(AllData), levels,
#                  cmap=cm.get_cmap(cmap, len(levels)-1),
#                  norm=norm)
#plt.clabel(CS, inline=1, fontsize=10)
#plt.gcf().autofmt_xdate()
#
##CS = plt.contour(AllTimes, AllAlts, np.transpose(AllData), levels,
##                 norm=norm)
#
#plt.ylabel('Altitude (km)')
#xtitle = AllTimes[0].strftime('%b %d, %Y %H:%M:%S')
#xtitle = xtitle+' to '
#xtitle = xtitle+AllTimes[-1].strftime('%b %d, %Y %H:%M:%S')
#plt.xlabel(xtitle)
#
##CS.set_title()
#
