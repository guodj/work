...
norm_e = cm.colors.Normalize(vmax=5e11, vmin=0)
cmap_e = cm.get_cmap('Blues')
norm_vi = cm.colors.Normalize(vmax=800, vmin=-800)
cmap_vi = cm.get_cmap('seismic')
...
for file in filelist:
    ...
    d2d_e = np.array(data[33][iLon, sLat:eLat, sAlt:eAlt])

    ax3 = fig.add_subplot(413)

    ax3.set_xlim(minLat, maxLat)
    ax3.set_ylim(minAlt, maxAlt)

    cax3 = ax3.pcolormesh(Lats[sLat:eLat], Alts[sAlt:eAlt], np.transpose(d2d_e), norm=norm_e, cmap=cmap_e)
    ax3.grid(True)

    cbar3 = fig.colorbar(cax3)
    cbar3.set_label('e-', rotation=90)

    d2d_vi = np.array(data[36][iLon, sLat:eLat, sAlt:eAlt])

    ax4 = fig.add_subplot(414)

    ax4.set_xlim(minLat, maxLat)
    ax4.set_ylim(minAlt, maxAlt)

    levels = [-800, -700, -600, -500, -400, -300, -200, -100, 0, 100, 200, 300, 400, 500, 600, 700, 800]
    cax4 = ax4.contour(Lats[sLat:eLat], Alts[sAlt:eAlt], np.transpose(d2d_vi), norm=norm_vi, cmap=cmap_vi,
                       levels=levels)
    ax4.grid(True)

    cbar4 = fig.colorbar(cax4)
    cbar4.set_label('Vi(east)', rotation=90)
