...
# Write imf.dat
dim = time.shape
imfby1 = np.ones(dim)*5+np.random.normal(0, 1, dim) # run1
...
# Write imf.dat
dim = time.shape
imfby3 = np.ones(dim)*-5+np.random.normal(0, 1, dim) # run3
reversalt = etime-pd.Timedelta('24hour')
mm = time>reversalt
imfby4 = imfby3.copy()
imfby4[mm] = imfby4[mm]+10
imfbz = np.ones(dim)*-1+np.random.normal(0, 1, dim)
imfbx = np.zeros(dim)
vx = np.ones(dim)*-450
vy = np.zeros(dim)
...
den = np.ones(dim)*5
tem = np.ones(dim)*50000
fimf3 = open(homepath+'/tmp/imf3.dat', 'w')
fimf3.write('#START\n')
for k00, k0 in enumerate(time):
    # Date and Time
    fimf3.write('{:5d}{:3d}{:3d}{:3d}{:3d}{:3d}{:4d}'.format(
            k0.year, k0.month, k0.day, k0.hour,
            k0.minute, k0.second, k0.microsecond))
    # IMF Bx, By, Bz
    fimf3.write('{:8.2f}{:8.2f}{:8.2f}'.format(
            imfbx[k00], imfby3[k00], imfbz[k00]))
    # Solar wind velocity, Vx, Vy, Vz
    fimf3.write('{:9.2f}{:9.2f}{:9.2f}'.format(
            vx[k00], vy[k00], vz[k00]))
    # Solar wind density and temperature
    fimf3.write('{:9.2f}{:13.1f}'.format(den[k00], tem[k00]))
    fimf3.write('\n')
fimf3.close()

fimf4 = open(homepath+'/tmp/imf4.dat', 'w')
fimf4.write('#START\n')
for k00, k0 in enumerate(time):
    # Date and Time
    fimf4.write('{:5d}{:3d}{:3d}{:3d}{:3d}{:3d}{:4d}'.format(
            k0.year, k0.month, k0.day, k0.hour,
            k0.minute, k0.second, k0.microsecond))
    # IMF Bx, By, Bz
    fimf4.write('{:8.2f}{:8.2f}{:8.2f}'.format(
            imfbx[k00], imfby4[k00], imfbz[k00]))
    # Solar wind velocity, Vx, Vy, Vz
    fimf4.write('{:9.2f}{:9.2f}{:9.2f}'.format(
            vx[k00], vy[k00], vz[k00]))
    # Solar wind density and temperature
    fimf4.write('{:9.2f}{:13.1f}'.format(den[k00], tem[k00]))
    fimf4.write('\n')
fimf4.close()
#----------------------------------------
# Write sme.dat
...
# display results
# Run3
fig3, ax3 = plt.subplots(3, 1, sharex=True, figsize=(7, 8))
plt.sca(ax3[0])
plt.title('Run3')
plt.plot(time, imfby3, 'b', time, imfbz, 'r')
hline = [5, -5, -1]
[plt.axhline(k, linestyle='--', color='gray') for k in hline]
vline = pd.date_range(btime, etime, freq='1D')
[plt.axvline(k, linestyle='--', color='gray') for k in vline]
...
plt.xlabel('Hours from '+btime.strftime('%Y-%m-%d %H:%M:%S'))
# Run4
fig4, ax4 = plt.subplots(3, 1, sharex=True, figsize=(7, 8))
plt.sca(ax4[0])
plt.title('Run4')
plt.plot(time, imfby4, 'b', time, imfbz, 'r')
hline = [5, -5, -1]
[plt.axhline(k, linestyle='--', color='gray') for k in hline]
vline = pd.date_range(btime, etime, freq='1D')
[plt.axvline(k, linestyle='--', color='gray') for k in vline]
