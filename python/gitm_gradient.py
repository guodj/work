...
def calc_gradient(g, var, name, component='radial'):
    ...
    Input: g          = gitm data
           ...
           component  = 'radial', 'north' or 'east'.
                        On which direction the gradient is calculated (default 'radial')
           name       = pick a name for the gradient of `var`.
    Output: g with `name` added
    '''
    from spacepy.datamodel import dmarray
    Re = 6371*1000 # Earth radius, unit: m
    lat, lon, alt, zdata = (g[k] for k in ['Latitude', 'Longitude', 'Altitude', var])
    if 'radial' in component.lower():
        grad = np.gradient(zdata, axis=2) / np.gradient(alt, axis=2)
    if 'north' in component.lower(): # directions of north and polar angle are opposite.
        grad = (1.0/(Re+alt))*np.gradient(zdata, axis=1) / np.gradient(lat, axis=1)
    ...
    g[name] = dmarray(grad,
                      attrs={'units':'None',
                             'scale':'linear',
                             'name':g[var].attrs['name']+' '+component+' component gradient'})
...
if __name__ == '__main__':
    g = gitm.GitmBin('/home/guod/WD2T/run_imfby/run1/data/3DALL_t100323_012500.bin')
