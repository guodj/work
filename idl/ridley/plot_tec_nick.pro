...
filelist = findfile('tec_gitm_compare_201003*.txt')
...
for iFile = 0, nFiles-1 do begin
   ...
   for iPt = 0, nPts-1 do begin
      ...
      if (FINITE(tmp(4)) and FINITE(tmp(2)) ) then begin
         ;print,  tmp(4)
        ; stop
...
maxrange = 30.0
contour_circle_nick, reform(tec(*,*,iModel)), lts, lats, $
   ...
   yp = yc + yr*sin(!pi/4)
   xyouts, xp, yp, 'North', $
           /norm, charsize = 1.3, align = 0.5, orient = 45
   
   xp = xc - xr*sin(!pi/4)*1.12
   yp = yc - yr*sin(!pi/4)*1.12
   xyouts, xp, yp, '60!Uo!N', $
           /norm, charsize = 1.3, align = 0.5
   
   xp = xc - xr*sin(!pi/4)*0.76
   yp = yc - yr*sin(!pi/4)*0.76
   xyouts, xp, yp, '70!Uo!N', $
           /norm, charsize = 1.3, align = 0.5
   
   xp = xc - xr*sin(!pi/4)*0.43
   yp = yc - yr*sin(!pi/4)*0.43
   xyouts, xp, yp, '80!Uo!N', $
           /norm, charsize = 1.3, align = 0.5
...
;        /norm, charsize = 1.3, align = 0.0

xp = pos(2)*0.66
yp= yp*1.02
xyouts, xp, yp, mo+model, /norm, charsize=1.7, align=1.0
