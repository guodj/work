...
psfile_bias = psfile_bias+'_'+ymd+'-'+ymd2+'.ps'
...
low = where(abs(Lats) le 30,c)
mid = where(abs(Lats) lt 60 and abs(Lats) gt 30,c)
hi = where(abs(Lats) ge 60,c)

rmseLOW = sqrt(mean((ChampAve(low)-gitmAve(low))^2))
rmsdLOW = sqrt(mean((ChampAve(low))^2))
nrmsLOW = rmseLOW/rmsdLOW * 100.0
pdifLOW = mean((ChampAve(low)-gitmAve(low))/ChampAve(low)) * 100.0

rmseMID = sqrt(mean((ChampAve(mid)-gitmAve(mid))^2))
rmsdMID = sqrt(mean((ChampAve(mid))^2))
nrmsMID = rmseMID/rmsdMID * 100.0
pdifMID = mean((ChampAve(mid)-gitmAve(mid))/ChampAve(mid)) * 100.0

rmseHI = sqrt(mean((ChampAve(hi)-gitmAve(hi))^2))
rmsdHI = sqrt(mean((ChampAve(hi))^2))
nrmsHI = rmseHI/rmsdHI * 100.0
pdifHI = mean((ChampAve(hi)-gitmAve(hi))/ChampAve(hi)) * 100.0
...
xyouts, t3, max(yrange)*(-0.75), 'Low nRMS: '+string(nrmsLOW, format = '(f5.1)')

t3 = (etr-btr)*0.35

xyouts, t3, max(yrange)*(-0.75), 'Mid nRMS: '+string(nrmsMID, format = '(f5.1)')

t3 = (etr-btr)*0.65

xyouts, t3, max(yrange)*(-0.75), 'High nRMS: '+string(nrmsHI, format = '(f5.1)')
...
plot, Gitmtime-stime, ChampAve, yrange = yrange*1.2, pos = pos, $
      ...
      thick = 3, /noerase
oplot, gitmtime-stime, gitmAve, linestyle = 2, thick = 3
oplot, gitmtime-stime, gitmAveBIAS, linestyle = 3, thick = 3
...
oplot, [t1,t2], max(yrange)*[1.05,1.05], thick = 3, linestyle = 3
xyouts, t3, max(yrange)*1.05, 'GITM - bias'

;--------- nRMS ----------

d = ChampAve-gitmAveBIAS
yr = [-maxi,maxi]
...
rmse = sqrt(mean((ChampAve-gitmAveBIAS)^2))
rmsd = sqrt(mean((ChampAve)^2))
nrms = rmse/rmsd * 100.0
    
pdif = mean((ChampAve-gitmAveBIAS)/ChampAve) * 100.0
...
;--------- low, mid, high lat %Diffs ----------


rmseLOW = sqrt(mean((ChampAve(low)-gitmAveBIAS(low))^2))
rmsdLOW = sqrt(mean((ChampAve(low))^2))
nrmsLOW = rmseLOW/rmsdLOW * 100.0
