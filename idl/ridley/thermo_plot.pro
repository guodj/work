...
if (n_elements(portrait) eq 0) then portrait = 0 else portrait = 1
...
;    endif
    ...
    xgrids = nlons
    location = string(alt(0,0,selset),format='(f5.1)')+' km alt'
endif
...
if yeswrite_cnt eq 1 then begin
    ...
    if (portrait eq 1) then setdevice,psfile,'p',5,0.95 $
    else setdevice,psfile,'l',5,0.6
    ...
    if (cnt1) then begin 
        ...
        if (not polar) then begin
            ...
            plot, [-mr, mr], [-mr, mr], pos=pos, $
              ...
              title=variable+' at '+location+' '+strmid(stime,0,15)+' UT'
...
;        oplot, [npx],[npy], psym = 4, symsize = 3, thick = 10
        ...
        if (n_elements(plotpole) gt 0) then begin
;           print, northpolelat, southpolelat
           if (npolar eq 1) then begin
