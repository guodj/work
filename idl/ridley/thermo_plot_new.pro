...
print, backup_xrange[1], default_xrange[1]
...
levelSecond = findgen(31)/30.0*(maxiSecond-miniSecond) + miniSecond
...
;    endelse
   ...
   if (polar) then begin
      ...
      if (n_elements(plotpkr) gt 0) then begin
         pkr_lat =  65.0
         pkr_lon = 212.0
         npx = (90.0-pkr_lat) * $
               cos((pkr_lon+utrot)*!pi/180.0 - !pi/2.0)
         npy = (90.0-pkr_lat) * $
               sin((pkr_lon+utrot)*!pi/180.0 - !pi/2.0)
         oplot, [npx],[npy], psym = 4, symsize = 3, thick = 10
      endif

      if (n_elements(plotsquare) gt 0) then begin
         lat_center = plotsquare(0)
         lon_center = plotsquare(1)
         dlat = plotsquare(2)
         dlon = plotsquare(3)

         minlat = lat_center - dlat/2.0
         minlon = lon_center - dlon/2.0
         maxlat = minlat + dlat
         maxlon = minlon + dlon

         sqlats = [fltarr(dlon)+minlat, $
                   minlat, maxlat, $
                   fltarr(dlon)+maxlat, $
                   maxlat, minlat]
         sqlons = [findgen(dlon)+minlon, $
                   maxlon, maxlon, $
                   maxlon-findgen(dlon), $
                   minlon, minlon]

         npx = (90.0-sqlats) * $
               cos((sqlons+utrot)*!pi/180.0 - !pi/2.0)
         npy = (90.0-sqlats) * $
               sin((sqlons+utrot)*!pi/180.0 - !pi/2.0)
         oplot, [npx],[npy], thick = 5
...
;       if mini eq maxi then maxi=mini+1
      ...
      title = variable+' at '+location+' at '+stime

      contour,datatoplot,x,y,POS=pos,$
              levels=levels,xstyle=1,ystyle=1,$
              xrange=xrange,yrange=yrange,$
              title=title,c_colors=clevels,$
              xtitle=xtitle,ytitle=ytitle,/cell_fill,/NOERASE
